 ...
 * messages, which brief §9 bans and which Brandon did not ask for. So the folders below are **views
 * over the one pool**: a message can appear in Store 456 and in Steritech, because it is a Steritech
 * message about Store 456, and it is still in the Inbox as well.
 *
 ...
 * A matcher over subject lines was the obvious shape and it is worse: "Store 456" appears in two
 * subjects and in one body, "0120 PINEVILLE" is written as "120 Pineville" by the one human who
 ...
 * an empty panel, or "Store 456 3" onto a list with two dots in it, is the app contradicting itself
...
object GridlinkSampleFolders {
    ...
    private val contents: Map<String, List<String>> = mapOf(
        ...
        "ops-456" to listOf("fill-ecolab", "steritech-cap", "fill-steritech-followup"),
        "ops-belmont" to listOf("fill-dl-truck", "alta-belmont"),
        "ops-pineville" to listOf("fill-alta-pineville", "rivera-callout"),
        "ops-midtown" to listOf("steritech-pest", "fill-permit"),
        "ops-randolph" to listOf("fill-facilities", "alta-randolph"),
        "vendors" to listOf("fill-duke", "fill-insurance"),
        "vendor-ecolab" to listOf("fill-ecolab"),
        "vendor-steritech" to listOf("steritech-cap", "steritech-pest", "fill-steritech-followup"),
        "vendor-sysco" to listOf("fill-sysco"),
        "people" to listOf(
            "fill-dl-truck",
            "fill-tperez",
            "fill-jeff-store",
        ...
        "receipts" to listOf("fill-sysco", "fill-duke", "fill-insurance", "fill-permit"),
    )
     ...
     * somebody tapped it, which is the one moment it is guaranteed to be in front of Brandon.
     */
    init {
 ...
 * sent; inside Vendors > Steritech every message is a robot, and collapsing them would turn the whole
