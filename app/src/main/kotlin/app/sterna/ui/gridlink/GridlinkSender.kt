...
/**
 * The real one: Sterna's persistent outbox.
 *
...
class GridlinkOutboxSender(
    ...
    override fun check(request: GridlinkComposeRequest): String? {
        ...
        if (accounts.load() == null) {
            ...
            // installs the stock Sterna launcher icon beside this one and they share an account
            // store, so the account is one icon away rather than one screen away.
            return "No account yet. Sign in with the Sterna icon first, then send from here."
        }
        ...
        // (duke-energy.com, ecolab.com, altametrics.com, charlottewater.org). It exists to make the
        // screens legible, and it was never addressable. With a live outbox behind the button,
        // tapping send on the demo reply would put mail from Brandon's own server into a stranger's
