 ...
 * The zone-meridian longitude proxy is an approximation with a KNOWN size, and Charlotte is close
 * to the worst case for it: the city sits at -80.84° and Eastern's meridian is -75°, so every
...
class GridlinkSolarTest {
    ...
    private fun dayLengthMinutes(date: LocalDate): Long {
        val sun = gridlinkSunTimes(date, charlotte)
    ...
    @Test
    fun `charlotte june sunset is in the evening`() {
        val sun = gridlinkSunTimes(LocalDate.of(2026, 6, 21), charlotte)
        // Published for Charlotte NC: sunrise 06:12, sunset 20:38 EDT. Computed lands ~23 min early
    ...
    @Test
    fun `charlotte december sunset is in the afternoon`() {
        val sun = gridlinkSunTimes(LocalDate.of(2026, 12, 21), charlotte)
        // Published for Charlotte NC: sunrise 07:29, sunset 17:15 EST.
    ...
    fun `dusk moves more than three hours across the year`() {
        ...
        val june = gridlinkSunTimes(LocalDate.of(2026, 6, 21), charlotte).sunset!!
        val december = gridlinkSunTimes(LocalDate.of(2026, 12, 21), charlotte).sunset!!
    ...
    fun `longitude is the standard offset in both january and july`() {
        ...
        assertEquals(
            -75.0,
            gridlinkLongitudeForZone(LocalDate.of(2026, 1, 15), charlotte),
            0.001,
        )
        assertEquals(
            -75.0,
            gridlinkLongitudeForZone(LocalDate.of(2026, 7, 15), charlotte),
            0.001,
        )
    }
    ...
    fun `midday is day and the small hours are oled`() {
        assertEquals(GridlinkMode.DAY, gridlinkModeAt(charlotteAt(2026, 6, 21, 12, 0)))
        assertEquals(GridlinkMode.OLED, gridlinkModeAt(charlotteAt(2026, 6, 21, 3, 0)))
    }

    @Test
    fun `eight pm is day in june and night in december`() {
        // The exact hour the old fixed ladder got wrong, in both directions.
        assertEquals(GridlinkMode.DAY, gridlinkModeAt(charlotteAt(2026, 6, 21, 20, 0)))
        assertEquals(GridlinkMode.NIGHT, gridlinkModeAt(charlotteAt(2026, 12, 21, 20, 0)))
    }

    @Test
    fun `december dusk turns the palette over before six`() {
        assertEquals(GridlinkMode.DAY, gridlinkModeAt(charlotteAt(2026, 12, 21, 16, 30)))
        assertEquals(GridlinkMode.NIGHT, gridlinkModeAt(charlotteAt(2026, 12, 21, 17, 30)))
    }
    ...
    fun `oled starts at eleven regardless of the season`() {
        ...
        assertEquals(GridlinkMode.NIGHT, gridlinkModeAt(charlotteAt(2026, 6, 21, 22, 59)))
        assertEquals(GridlinkMode.OLED, gridlinkModeAt(charlotteAt(2026, 6, 21, 23, 1)))
        assertEquals(GridlinkMode.OLED, gridlinkModeAt(charlotteAt(2026, 12, 21, 23, 1)))
    }
    ...
    private fun charlotteAt(year: Int, month: Int, day: Int, hour: Int, minute: Int) =
        ZonedDateTime.of(year, month, day, hour, minute, 0, 0, charlotte)
}
