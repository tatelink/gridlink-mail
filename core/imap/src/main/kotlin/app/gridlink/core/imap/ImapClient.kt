...
class ImapSession(private var socket: Socket) : Closeable {
     ...
     * which some client (including Gridlink up to 1.4.3, which sent folder names raw) may have
...
internal fun parseListFolders(untagged: List<List<Any?>>): List<ImapFolder> =
    untagged.mapNotNull { resp ->
        ...
        // conforming: some client sent raw UTF-8 (Gridlink itself did, up to 1.4.3 — see the
