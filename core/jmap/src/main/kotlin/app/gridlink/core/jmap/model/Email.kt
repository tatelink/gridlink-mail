...
data class Email(
     ...
     * (see [app.gridlink.core.jmap.DownloadLimits]). They are not fetched, so the reader has to
    ...
    fun oversizedInlineImageCount(): Int = inlineImageParts().count {
        !app.gridlink.core.jmap.DownloadLimits.allows(
            it.size,
            app.gridlink.core.jmap.DownloadLimits.INLINE_IMAGE_MAX_BYTES,
        )
    }
