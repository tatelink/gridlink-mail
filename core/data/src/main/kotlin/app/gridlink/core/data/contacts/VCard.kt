...
data class ParsedContact(
     ...
     * migrated data runs `FN:McLane Foodservice` / `N:Foodservice;McLane;;;` / `ORG:McLane
     * Foodservice;`: the exporter split a company name down the middle into a fake surname and a
     * fake first name. Trusting `N` there files McLane Foodservice under F, next to nothing else it
...
object VCard {
    ...
    private fun build(lines: List<ContentLine>): ParsedContact? {
        ...
        val card = ParsedContact(
            ...
            // ORG is structured (`ORG:McLane Foodservice;` is a name plus an empty unit), so the
