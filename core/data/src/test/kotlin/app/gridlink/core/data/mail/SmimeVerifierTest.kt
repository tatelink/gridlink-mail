...
class SmimeVerifierTest {
    ...
    fun `a message signed by its sender names the sender`() {
        val raw = signedMessage(signerAddress = "dana@example.com", from = "dana@example.com")
        val envelope = MimeParser.detectSmime(raw)
        assertNotNull(envelope)
        assertEquals(SmimeKind.DETACHED, envelope!!.kind)
        val verdict = SmimeVerifier.verify(envelope, "dana@example.com")
        ...
        assertEquals("dana@example.com", verdict.signerEmail)
        assertEquals("Dana Sender", verdict.signerName)
    }
    ...
    fun `a message altered after signing does not verify`() {
        ...
        assertEquals(SmimeStatus.INVALID, SmimeVerifier.verify(envelope, "dana@example.com").status)
    }
    ...
    fun `a whole message wrapped in its signature is read and verified`() {
        val raw = opaqueSignedMessage(signerAddress = "dana@example.com")
        val envelope = MimeParser.detectSmime(raw)!!
    ...
    fun `a signature blob that is not a signature claims nothing`() {
        ...
        val envelope = MimeParser.detectSmime(raw)!!
        assertEquals(SmimeStatus.UNSUPPORTED, SmimeVerifier.verify(envelope, "dana@example.com").status)
    }
    ...
    fun `Outlook's own spelling of the signature part is still a signature`() {
        ...
        val raw = signedMessage("dana@example.com", "dana@example.com")
            .replace("application/pkcs7-signature", "application/x-pkcs7-signature")
        val envelope = MimeParser.detectSmime(raw)
        assertNotNull(envelope)
        assertEquals(SmimeStatus.UNTRUSTED, SmimeVerifier.verify(envelope!!, "dana@example.com").status)
    }

    @Test
    fun `a message with no S-MIME in it is left alone`() {
        val raw = "From: dana@example.com\r\nSubject: Hi\r\nContent-Type: text/plain\r\n\r\nHello.\r\n"
        assertNull(MimeParser.detectSmime(raw))
    }
    ...
    private fun opaqueSignedMessage(signerAddress: String): String {
        ...
        return buildString {
            append("From: Dana Sender <$signerAddress>\r\n")
