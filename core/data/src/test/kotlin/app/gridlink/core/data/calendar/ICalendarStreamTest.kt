...
class ICalendarStreamTest {
    ...
    private val windowsZoned = wrap(
        ...
        END:VTIMEZONE
        BEGIN:VEVENT
        UID:pineville-urology
        SUMMARY:Pineville Urology
    ...
    fun `a Windows time zone name resolves to a real zone, daylight saving included`() {
        ...
        // this test pass in Charlotte and fail everywhere else, which is why the fallback here is
    ...
    fun `a UTC stamp is converted, not read as a wall time`() {
        val event = ICalendarStream.parse(
            wrap(
                ...
                BEGIN:VEVENT
                UID:avance-care
                SUMMARY;LANGUAGE=en-us:Avance Care Established - Patient Psychiatry with Bro
                 ok Evans
                DTSTART:20261021T170000Z
                DTEND:20261021T181500Z
                LOCATION;LANGUAGE=en-us:5821 Fairview Rd\, Ste 200 Charlotte NC 28209-3649
                END:VEVENT
                """.trimIndent(),
            ),
            eastern,
        ...
        assertEquals(
            "Avance Care Established - Patient Psychiatry with Brook Evans",
            event.summary,
        )
        assertEquals("5821 Fairview Rd, Ste 200 Charlotte NC 28209-3649", event.location)
    ...
    fun `a reminder inside the event is not mistaken for the event`() {
        ...
        assertEquals("Area Coach one to one", event.summary)
    }
    ...
    fun `an unresolvable zone falls back to the offset the file carries`() {
        ...
        // not in Charlotte is simply a different appointment.
