...
- 🔜 **OAuth2 + XOAUTH2 for mainstream IMAP providers** *(1.1)* — the OAuth flow above targets OAuth-discovery JMAP servers; it does **not** cover Outlook/Microsoft or Gmail, which have disabled basic-auth IMAP and require provider-specific OAuth2 with the **XOAUTH2** SASL mechanism. Needs: a registered Microsoft/Google OAuth client (public client id, like Thunderbird/K-9), the Microsoft/Google auth-code or device flow, and an `AUTHENTICATE XOAUTH2` path in the IMAP **and** SMTP clients (currently password `LOGIN` only). Until then: **no Outlook** (basic auth is dead there — neither a password nor an app password works); Gmail/Yahoo/iCloud/Fastmail work with an **app password**.
