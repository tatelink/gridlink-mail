    ...
    static class TransferThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                Log.e("OpenPgpApi", "IOException when writing to out", e);
            } finally {
                try {
                    mIn.close();
        ...
        public void run() {
            try {
                ...
                if (dataSource.isCancelled()) {
                    Log.d("OpenPgpApi", "Stopped writing because operation was cancelled.");
                } else if (isIOExceptionCausedByEPIPE(e)) {
                    Log.d("OpenPgpApi", "Stopped writing due to broken pipe (other end closed pipe?)");
                } else {
                    Log.e("OpenPgpApi", "IOException when writing to out", e);
                }
            } finally {
        ...
        public void run() {
            try {
                ...
                if (isIOExceptionCausedByEPIPE(e)) {
                    Log.e("OpenPgpApi", "Stopped read due to broken pipe (other end closed pipe?)");
                } else {
                    Log.e("OpenPgpApi", "IOException while reading from in", e);
                }
                sinkResult = null;
            } finally {
                try {
